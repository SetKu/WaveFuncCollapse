...
pub fn dimensions_of<T>(input: &Vec<Vec<T>>) -> Vector2<usize> {
    Vector2::new(input.len(), if input.len() > 0 { input[0].len() } else { 0 })
}
...
pub fn roll<T>(input: &mut Vec<Vec<T>>, shifts: usize, x_axis: bool, y_axis: bool) where T: Clone {
...
pub fn all_possible_shifts<T>(input: Vec<Vec<T>>) -> Vec<Vec<Vec<T>>> where T: Clone {
    ...
    let total_size = std::mem::size_of::<T>() * input_size.x * input_size.y * shifts * 3;
